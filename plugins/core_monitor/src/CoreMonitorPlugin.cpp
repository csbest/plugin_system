...
extern "C" void DestoryPluginFactory(pluginfw::IPluginFactory* factory)
{
    (void)factory;
}
...
namespace pluginfw
{
    ...
    std::string CoreMonitorPlugin::name() const { return name_; }
    std::string CoreMonitorPlugin::version() const { return version_; }
    ...
    bool CoreMonitorPlugin::monitorLoop()
    {
        ...
        while ( running_)
        {
            ...
            else
            {
                ...
                state.status_msg = disk_usage > 90.0 ? "CRITICAL" : ( disk_usage > 80 ? "WARNING" : "NORMAL");
            }
